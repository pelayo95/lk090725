...
const SettingsPage = () => {
    ...
    const settingCategories = useMemo(() => [
        { name: 'Gestión de Acceso', icon: <Users className="w-5 h-5 flex-shrink-0"/>, items: [{ id: 'roles', label: 'Roles y Permisos', permission: 'config_puede_gestionar_roles', component: RoleManagementPage, icon: <ShieldCheck className="w-5 h-5 flex-shrink-0" /> }] },
        { name: 'Configuración de Denuncias', icon: <FileText className="w-5 h-5 flex-shrink-0"/>, items: [
            { id: 'form', label: 'Formulario Público', permission: 'config_puede_gestionar_formularios', component: FormSettings, icon: <ClipboardList className="w-5 h-5 flex-shrink-0" /> },
            { id: 'declaration', label: 'Declaración de Veracidad', permission: 'config_puede_gestionar_declaracion', component: DeclarationSettings, icon: <AlertTriangle className="w-5 h-5 flex-shrink-0" /> },
            { id: 'timeline', label: 'Líneas de Tiempo', permission: 'config_puede_gestionar_timelines', component: TimelineSettings, icon: <Clock className="w-5 h-5 flex-shrink-0" /> },
        ]},
        { name: 'Plantillas y Contenidos', icon: <Brush className="w-5 h-5 flex-shrink-0"/>, items: [
            { id: 'templates', label: 'Plantillas de Comunicación', permission: 'config_puede_gestionar_plantillas', component: CommunicationTemplatesSettings, icon: <MessageSquarePlus className="w-5 h-5 flex-shrink-0" /> },
            { id: 'measures', label: 'Medidas por Defecto', permission: 'config_puede_gestionar_medidas_defecto', component: MeasuresSettings, icon: <Shield className="w-5 h-5 flex-shrink-0" /> },
            { id: 'doc_categories', label: 'Categorías de Documentos', permission: 'documentacion_puede_gestionar', component: DocumentationSettings, icon: <LayoutList className="w-5 h-5 flex-shrink-0" /> },
        ]},
        { name: 'Módulos y Automatización', icon: <SlidersHorizontal className="w-5 h-5 flex-shrink-0"/>, items: [
            { id: 'accused_portal', label: 'Portal del Denunciado', permission: 'config_puede_gestionar_portal_denunciado', component: AccusedPortalSettings, icon: <Eye className="w-5 h-5 flex-shrink-0" /> },
            { id: 'notifications', label: 'Reglas de Notificación', permission: 'config_puede_gestionar_notificaciones', component: NotificationSettings, icon: <Bell className="w-5 h-5 flex-shrink-0" /> },
        ]},
    ], []);
                            ...
                            <div key={category.name}>
                                ...
                                <div className="space-y-1">
                                    {category.items.map(item => {
                                        ...
                                        let buttonClasses = 'w-full text-left text-sm px-3 py-2 rounded-md transition-colors flex items-center gap-3 justify-center md:justify-start';
                                        
                                        if (isSidebarExpanded) {
                                            buttonClasses += isActive ? ' bg-indigo-100 text-indigo-700 font-semibold' : ' text-slate-600 hover:bg-slate-100';
                                        } else {
                                            // Cuando está colapsado, usamos un fondo oscuro para que los íconos blancos resalten.
                                            buttonClasses += isActive ? ' bg-indigo-600 text-white' : ' bg-slate-800 text-slate-300 hover:bg-slate-700';
                                        }
