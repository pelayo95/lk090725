...
const SettingsPage = () => {
    ...
    // --- INICIO DE LA MODIFICACIÓN: Nueva estructura de navegación ---
    ...
    // Filtra las categorías y los items basados en los permisos del usuario
    ...
    // Define el estado activo, inicializándolo con el primer item visible
    const [activeSetting, setActiveSetting] = useState(() => visibleCategories[0]?.items[0]?.id || '');
    
    // Encuentra el componente activo para renderizar
                            ...
                            <div className="space-y-1">
                                {category.items.map(item => (
                                    <button 
                                        ...
                                        className={`w-full text-left text-sm px-3 py-2 rounded-md transition-colors ${activeSetting === item.id ? 'bg-indigo-100 text-indigo-700 font-semibold' : 'text-slate-600 hover:bg-slate-100'}`}
                                    >
                                        {item.label}
                                    </button>
                                ))}
